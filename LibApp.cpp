...
namespace sdds {
	...
	void LibApp::load()
	{
		...
		char kind;
		for (int i = 0; i < SENECA_LIBRARY_CAPACITY and file; i++)
		{
			file >> kind;
			file.ignore();
	...
	void LibApp::returnPub()
	{
		...
		if (libref > 0)
		{
			if (confirm("Return Publication?"))
			{
				...
				if (day > SENECA_MAX_LOAN_DAYS)
				{
					double penalty = (day - SENECA_MAX_LOAN_DAYS) * 0.5;
					cout << fixed << setprecision(2) << "Please pay $" << penalty << " penalty for being " << (day - SENECA_MAX_LOAN_DAYS) << " days late!" << endl;
				}
